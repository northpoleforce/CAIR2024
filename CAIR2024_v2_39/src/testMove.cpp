...
int main(int argc, char **argv)
{
    ...
    getTasks(task, task1st, task2nd, cam0);
    ...
    // 启动视觉导航
    PIDController pidYaw(0.015, 0, 0);                                                   // Yaw轴PID
    ...
    while (true)
    {
        ...
        if (loadFrontCamera && !task[5].arrived)
        {
            ...
            cv::Mat frontResult = frame.clone();
            taskDetector.detect(frame, frontResult, taskDetected); // 检测任务标签
        }
        else
        {
